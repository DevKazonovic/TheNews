...
class StoriesStateListAdapter(private var data : List<Pair<Int,Boolean>>)
    : RecyclerView.Adapter<StoriesStateListAdapter.MyViewHolder>()  {
    ...
    fun updateList(currentSelectedPos : Int){
        data = data.map { item -> if(item.first==currentSelectedPos) Pair(item.first,true) else Pair(item.first,false) }
        notifyDataSetChanged()
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val inflater= LayoutInflater.from(parent.context)
        val view = inflater.inflate(R.layout.item_circle,parent,false)
